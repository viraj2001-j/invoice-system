...
export default function EditInvoicePopup({ invoice, isOpen, onClose }: any) {
  ...
  /**
   * 🟢 AUTO-DUE DATE LOGIC
   * Shifts Due Date 15 days forward when Issue Date changes
   */
  const handleDateChange = (newIssueDate: string) => {
    const date = new Date(newIssueDate);
    date.setDate(date.getDate() + 15);
    const newDueDate = date.toISOString().split('T')[0];
    
    const { subtotal, taxAmount, total } = handleCalculation(
      formData.items, 
      formData.taxRate, 
  ...
  /**
   * 🧮 CALCULATION ENGINE
   * Handles Subtotal, Tax, and toggles between Flat and Percentage discounts
   */
  const handleCalculation = (items: any[], taxRate: number, discountValue: number, discountType: string) => {
    const subtotal = items.reduce((sum, item) => sum + (item.amount || 0), 0);
    const taxAmount = (subtotal * (taxRate || 0)) / 100;
    
    let discountAmount = discountValue || 0;
  ...
  const handleSave = async () => {
    toast.promise(updateFullInvoiceAction(formData.id, formData), {
      loading: 'Syncing system records...',
      success: () => {
        onClose();
        return "Invoice records updated successfully.";
      },
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent 
        className="max-w-[95vw] md:max-w-[110vh] w-full max-h-[95vh] overflow-y-auto p-0 gap-0 border-none shadow-2xl"
      >
        {/* STICKY HEADER */}
        <DialogHeader className="p-6 bg-slate-950 text-white flex flex-row items-center justify-between space-y-0 sticky top-0 z-50">
        ...
        <div className="p-8 space-y-12 bg-white">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 space-y-1">
                  ...
                  <Input className="bg-slate-50" value={formData.company?.name || ""} onChange={(e) => setFormData({...formData, company: {...formData.company, name: e.target.value}})} />
                </div>
                <div className="space-y-1">
                  <Label className="text-[10px] font-bold text-blue-600">Project Name</Label>
                  <Input className="bg-blue-50/30" value={formData.company?.project || ""} onChange={(e) => setFormData({...formData, company: {...formData.company, project: e.target.value}})} />
                </div>
                <div className="space-y-1">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 space-y-1">
                  ...
                  <Input className="bg-slate-50" value={formData.client.name} onChange={(e) => setFormData({...formData, client: {...formData.client, name: e.target.value}})} />
                </div>
                <div className="space-y-1">
          ...
          <div className="bg-slate-50 p-6 border-y-2 border-black flex flex-wrap gap-8 items-end shadow-inner">
            <div className="flex-1 min-w-[150px] space-y-1">
              <Label className="text-[10px] font-black uppercase">Invoice No.</Label>
              <Input className="font-mono font-bold text-blue-600 bg-white" value={formData.invoiceNumber} onChange={(e) => setFormData({...formData, invoiceNumber: e.target.value})} />
            </div>
            ...
            <div className="flex-1 min-w-[150px] space-y-1 text-red-600">
              <Label className="text-[10px] font-black uppercase">Due Date (Net 15)</Label>
              <Input 
                type="date" 
                className="bg-white border-red-100 font-bold"
            ...
            <div className="flex-1 min-w-[120px] space-y-1">
              ...
              >
                <SelectTrigger className="bg-white font-bold border-slate-300">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b border-slate-200 pb-2">
              <h3 className="font-black uppercase text-xs tracking-widest flex items-center gap-2">
                <FileText size={16} /> Product / Service List
              </h3>
              <Button onClick={addItem} size="sm" className="bg-blue-600 text-[10px] font-bold px-4 h-7 uppercase tracking-widest">
                <Plus size={14} className="mr-1" /> Add Row
              </Button>
            </div>
            <div className="space-y-2">
              {formData.items.map((item: any, index: number) => (
                <div key={index} className="flex gap-4 items-center bg-slate-50/50 p-2 border border-slate-100 rounded">
                  <div className="flex-[5]"><Input placeholder="Description" className="bg-white" value={item.description} onChange={(e) => handleItemChange(index, 'description', e.target.value)} /></div>
                  <div className="w-24"><Input type="number" placeholder="Qty" className="bg-white" value={item.quantity || ""} onChange={(e) => handleItemChange(index, 'quantity', parseFloat(e.target.value))} /></div>
                  <div className="w-36"><Input type="number" placeholder="Rate" className="bg-white" value={item.rate || ""} onChange={(e) => handleItemChange(index, 'rate', parseFloat(e.target.value))} /></div>
                  <div className="w-40 text-right font-black font-mono text-slate-700">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 pt-6">
            <div className="space-y-6">
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase text-slate-500">Official Note</Label>
            ...
            <div className="bg-black text-white p-8 rounded-none border-l-8 border-blue-600 shadow-2xl space-y-6">
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center text-xs font-bold text-slate-500 uppercase">
                  <div className="flex items-center gap-1"><Percent size={12} className="text-blue-500" /> Tax %</div>
                  <div className="w-32">
                    <Input className="h-7 bg-slate-900 border-slate-700 text-right text-xs" type="number" 
                      value={formData.taxRate || ""} 
                      onChange={(e) => {
                        const rate = parseFloat(e.target.value) || 0;
                        const { subtotal, taxAmount, total } = handleCalculation(formData.items, rate, formData.discountValue, formData.discountType);
                ...
                <div className="flex justify-between items-center text-xs font-bold text-slate-500 uppercase">
                  ...
                  <div className="flex gap-1 w-48">
                    <Input className="h-7 bg-slate-900 border-slate-700 text-right text-xs flex-1" type="number" 
                      value={formData.discountValue || ""} 
                      onChange={(e) => {
                        const disc = parseFloat(e.target.value) || 0;
                        const { subtotal, taxAmount, total } = handleCalculation(formData.items, formData.taxRate, disc, formData.discountType);
                        setFormData({...formData, discountValue: disc, subtotal, taxAmount, total});
                      }} 
                    />
                    <select 
                        className="h-7 bg-slate-800 text-[10px] border-none rounded px-1 outline-none text-blue-400 font-bold"
                        value={formData.discountType}
                        onChange={(e) => {
                            const type = e.target.value;
                            const { subtotal, taxAmount, total } = handleCalculation(formData.items, formData.taxRate, formData.discountValue, type);
                            setFormData({...formData, discountType: type, subtotal, taxAmount, total});
                        }}
                    >
              ...
              <div className="border-t border-slate-800 pt-6 flex justify-between items-end">
                <div className="flex flex-col">
                  <span className="text-blue-500 text-[10px] font-black uppercase tracking-[0.2em]">Net Amount Payable</span>
                  <span className="text-4xl font-black italic tracking-tighter">{formData.currency} {formData.total.toLocaleString()}</span>
                </div>
        ...
        {/* STICKY FOOTER */}
        <DialogFooter className="p-6 bg-slate-100 border-t border-slate-200 sticky bottom-0 z-50">
          <Button variant="ghost" onClick={onClose} className="font-bold hover:bg-white">CANCEL</Button>
          <Button onClick={handleSave} className="bg-blue-600 hover:bg-blue-700 px-12 rounded-none font-black uppercase tracking-widest">
            <Save size={16} className="mr-2" /> SYNC UPDATES
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
